// initializing the game when all the content fully loads
...
function game() {
    ...
    // move the player around the earth in circular motion
    function move(e) {
        player.deg = Math.atan2(e.offsetX - (cW/2), -(e.offsetY - (cH/2)));
    }


    // firing action, bullets or game over
    function action(e) {
        e.preventDefault();
        if(playing) {
            var bullet = {
    ...
    // drawing and rotating the earth planet on canvas
    function planet() {
        ctx.save();
        ctx.fillStyle   = 'white';
        ctx.shadowBlur    = 100;
    ...
    // firing new asteroid with new values and co-ordinates
    ...
    // showing the explosion when bullet hit any asteroid
